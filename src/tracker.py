...
class BoxTracker(Tracker):
    ...
    def __init__(self, logger : ConsoleLogger, tracker_type = PARAMS['TRACKER'], mp_catchup = PARAMS['CATCHUP_MULTITHREAD'],
    ...
    def execute_catchup_mp(self, old_timestep, old_detections):
        ...
        self.logger.log_info(f'Launched {len(threads)} threads')
        assert len(objects_to_track) == 0, f'Threads did not completely fill objects_to_track: {objects_to_track}'
        assert len(threads) > 1, f'Threads length is {len(threads)}; mistake'
